...
$WS_SDS_PATCH     = 'http://'.get_option('sds_instancia').'.smartdealer.com.br/webservice/core.php?wsdl';
...
function MakePager(){
	...
	if (isset($_GET['pp'])) unset($_GET['pp']);
...
if (isset($_GET['pp'])) $WS_SDS_PAGE = addslashes($_GET['pp']);
...
$in = array(
		'pp' 		  => $WS_SDS_PAGE,  // -> (integer) Número da página requisitada
		'qtd_por_pp'  => $WS_SDS_PAGE_SIZE   // -> (integer) Quantidade de registros por página
		);	
...
if (isset($_POST['querer_x'])):
	...
	if (($_POST['mail'] == '') and ($_POST['tel']  == '')){
		$contato['msn']   = 'Atenção, sua mensagem não pode ser enviada!! <br/ ><br/ > <strong>Por favor preencha seu e-mail ou seu telefone!</strong>';
	...
	if ($_POST['nome'] == ''){
		$contato['msn']   = 'Atenção, sua mensagem não pode ser enviada!! <br/ ><br/ > <strong>Por favor preencha o seu nome!</strong>';
		$contato['class'] = 'ReturnMessageErro'; 
		$contato['show']  = true;	
		++$err_form;
		}
...
<script src="<?php echo $PLUGIN_URL ?>js/jquery.js"></script>
...
<link href="<?php echo $PLUGIN_URL ?>css/base.css" rel="stylesheet" type="text/css"/>
<link href="<?php echo $PLUGIN_URL ?>css/960.css" rel="stylesheet" type="text/css">
...
$('.euquero').click(function(){
	...
	html = html + '<tr>';
	html = html + '<td height="30" colspan="2" valign="top">Deixe seus dados que um de nosso vendedores entrará em contato!</td>';
	html = html + '</tr>';
	html = html + '<tr>';
...
<table height="100%" width="100%" id="conteudo" border="0" cellspacing="0" cellpadding="0">
  <tbody>

  <tr>
  
                
        <td valign="top" height="580" style="580px auto !important">
           
            <center>
                 <div id="pagina">
                      <style type="text/css">

.vtitulo{position: absolute; font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-family:10px; color:#881228;}
.iveiculo{float:left;padding:5px 9px 10px 0;height: 108px; border-left: solid 10px #FFF;position: relative;}
	...
	<form method="get" action="<?php echo 'http://'.$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF'];  ?>">
		...
		<div class="grid_12 top10 alpha">
            ...
            <select name="familia" id="familia" style="width:206px;">
				...
				$itens = $WS_SDS->call('GetFamilias', array('parametrosbusca' => $prametros)); foreach ($itens as $item): ?>
            ...
            <select name="cor" id="cor" style="width:206px;">
                <option value="" selected="selected">Escolha </option>
                <?php $itens = $WS_SDS->call('GetCores', array('parametrosbusca' => $prametros)); foreach ($itens as $item): ?>
                <option value="<?php echo urlencode($item['item']); ?>"><?php echo $item['item']; ?></option>
                <?php endforeach; ?>
            </select>
            </div>
            ...
            <select name="combustivel" id="combustivel" style="width:206px;">
                <option value="" selected="selected">Escolha </option>
                <?php $itens = $WS_SDS->call('GetCombustiveis', array('parametrosbusca' => $prametros)); foreach ($itens as $item): ?>
                <option value="<?php echo urlencode($item['item']); ?>"><?php echo $item['item']; ?></option>
                <?php endforeach; ?>
            </select>
            </div>
...
<div class="grid_16">
	...
	if(isset($a[0])) $modelo = $a[0]; 

	$cor = '';
	$a = explode('.', $a[1]);
	if(isset($a[0])) $cor = $a[0]; 

	$c='';     
	if($carro['status']=='A')$c='&reservado';
	if($carro['status']=='V')$c='&reservado';
	if($carro['status']=='CR')$c='&reservado';
	if(($carro['status']=='X') or ($carro['status']=='L')) $c='&vendido';
